...
if st.session_state.mode_chosen_flag:
    ...
    with input_col:
        ...
        with st.form("prediction_input_form", clear_on_submit=False):
            ...
            # --- IMAGE UPLOADER (MOVED BEFORE MANUAL OVERRIDE) ---
            if show_image_uploader:
                st.subheader("Image Input")
                current_uploaded_file = st.file_uploader("Upload coral image", type=["jpg","png","jpeg"], key="image_uploader")
                st.markdown("---")

            # FIX #2: MANUAL OVERRIDE / FULL MANUAL ENTRY - NOW OUTSIDE THE FORM
            # We'll handle this after the form
        ...
        # FIX #2: Manual data inputs OUTSIDE the form so arrows work
        if show_manual_override:
            ...
            if is_fusion:
                with st.expander("Click to manually input data (Highly Optional)"):
                    display_manual_inputs = True
            else: # Tabular-Only (Manual)
                display_manual_inputs = True

            if display_manual_inputs or not is_fusion:
                ...
                with c2:
                    ...
                    current_exposure_index = exposure_options.index(st.session_state.manual_features["Exposure"]) if st.session_state.manual_features["Exposure"] in exposure_options else 0
                    st.session_state.manual_features["Exposure"] = st.selectbox(
                        "Reef Exposure",
                        options=exposure_options,
                        index=current_exposure_index,
                        key="exposure_outer"
                    )
                ...
                with c3:
                    st.session_state.manual_features["SSTA"] = st.number_input(
                        "SSTA",
                        value=st.session_state.manual_features["SSTA"],
                    ...
                    st.session_state.manual_features["SSTA_DHW"] = st.number_input(
                        "SSTA DHW",
                        value=st.session_state.manual_features["SSTA_DHW"],
                ...
                with c4:
                    st.session_state.manual_features["TSA"] = st.number_input(
                        "TSA",
                        value=st.session_state.manual_features["TSA"],
                    ...
                    st.session_state.manual_features["TSA_DHW"] = st.number_input(
                        "TSA DHW",
                        value=st.session_state.manual_features["TSA_DHW"],
                        help="Thermal Stress Anomaly Degree Heating Weeks",
                        key="tsa_dhw_outer",
                        format="%.3f"
                    )

                # Copy manual features to current_override_features for submission
